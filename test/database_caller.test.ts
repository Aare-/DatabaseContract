...
import { DatabaseArtifacts, DatabaseBase, DatabaseCallerBase } from 'database';
...
contract('DatabaseCaller', accounts => {
  ...
  describe('#ctor', () => {
    it('should successfully create new database caller contract', async () => {
      assert.isNotNull(dbCallerContract);
    });
  });

  describe('#collectData', () => {
    it('should correctly collect data from empty database', async () => {
      const addressesList = await dbCallerContract.collectData(
        dContract.address
      );
      assert.deepEqual(addressesList, []);
    });

    it('should correctly collect data from populated database', async () => {
    ...
    it('should correctly collect data from different databases', async () => {
      anotherDContract = await DatabaseContract.new();

      await dContract.registerAddress(user1);
      await anotherDContract.registerAddress(user2);

      const addressesList1 = await dbCallerContract.collectData(
        dContract.address
      );
      const addressesList2 = await dbCallerContract.collectData(
        anotherDContract.address
      );
