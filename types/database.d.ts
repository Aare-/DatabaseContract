declare module 'database' {
  ...
  namespace database {
    ...
    interface DatabaseBase extends ContractBase {
      ...
      isAddressRegistered(addressToCheck: Address): Promise<boolean>;

      getAllAddresses(): Promise<Address[]>;

      deRegisterAll(options?: TransactionOptions): Promise<TransactionResult>;

      getNextAddress(predecessorAddress: Address): Promise<Address>;

      countAddresses(options?: TransactionOptions): Promise<BigNumber>;
    }

    interface DatabaseCallerBase extends ContractBase {
      collectData(databaseAddress: Address): Promise<Address[]>;
    }

    interface DatabaseContract extends Contract<DatabaseBase> {
      'new'(options?: TransactionOptions): Promise<DatabaseBase>;
    }

    interface DatabaseCallerContract extends Contract<DatabaseCallerBase> {
      'new'(options?: TransactionOptions): Promise<DatabaseCallerBase>;
    }
